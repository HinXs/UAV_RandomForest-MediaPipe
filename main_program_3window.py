...
def connect_to_esp32():
    global esp_connected, client_socket
    
    try:
        ESP32_IP = socket.gethostbyname(ESP32_HOST)
        print(f"🌐 Mencoba koneksi ke {ESP32_HOST} ({ESP32_IP})")
        
        client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        client_socket.settimeout(5)  # Timeout 2 detik untuk koneksi
        ...
        try:
            ...
            print("ℹ️ Simulasi akan tetap berjalan tanpa koneksi ESP32")
            if client_socket:
                client_socket.close()
            client_socket = None
        ...
        client_socket = None
# Panggil fungsi koneksi saat program dimulai
connect_to_esp32()
